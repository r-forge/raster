...
newBrick <- function(xmn=-180, xmx=180, ymn=-90, ymx=90, nrows=180, ncols=360, projection="+proj=longlat +datum=WGS84") {
	bb <- newBbox(xmn, xmx, ymn, ymx, projection)
	return(brickFromBbox(bb, nrows=nrows, ncols=ncols))
}

brickFromBbox <- function(boundingbox, nrows=1, ncols=1) {
