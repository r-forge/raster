...
if ( !isGeneric("corrLocal") ) {
	setGeneric("corrLocal", function(x, y, ...)
		standardGeneric("corrLocal"))
}


setMethod('corrLocal', signature(x='RasterLayer', y='RasterLayer'), 
	function(x, y, ngb=5, method = c("pearson", "kendall", "spearman"), filename='', ...) {
		...
		if (canProcessInMemory(x)) {
			...
			tr <- blockSize(out)
			pb <- pbCreate(tr$n, label='corrLocal', ...)
