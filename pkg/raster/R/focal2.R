...
focal2 <- function(x, w, fun, filename='', na.rm=FALSE, pad=FALSE, padValue=NA, NAonly=FALSE, ...) {
	...
	if (!is.null(dots$filter)) {
		warning('argument "filter" is ignored; use "w" instead')
	}
	if (!is.null(dots$ngb)) {
		warning('argument "ngb" is ignored; use "w" instead')
	}
	...
	if (canProcessInMemory(out)) {
		if (pad) {
			...
			if (gll) {
				v <- cbind(v[, (ncol(v)-f[2]+1):ncol(v)], v, v[, 1:f[1]])	
			} else {
				padCols <- matrix(padValue, nrow=nrow(out)+2*f[1], ncol=f[2])
				v <- cbind(padCols, v, padCols)
			}
			paddim <- as.integer(dim(v))
			if (dofun) {
				v <- .Call('focal_fun', as.vector(t(v)), w, paddim, narm, fun, e, NAOK=TRUE, PACKAGE='raster')
			} else {
