...
if (!isGeneric("layer")) {
	setGeneric("layer", function(x, ...)
		standardGeneric("layer"))
}	


setMethod('layer', signature(x='missing'), 
	function(nrows=180, ncols=360, xmn=-180, xmx=180, ymn=-90, ymx=90, projs="+proj=longlat +datum=WGS84") {
		ext <- extent(xmn, xmx, ymn, ymx)
		r <- layer(ext, nrows=nrows, ncols=ncols, projs=projs)
		return(r)
	}
)


setMethod('layer', signature(x='matrix'), 
	function(x, xmn=0, xmx=1, ymn=0, ymx=1, projs=NA) {
		r <- layer(ncols=ncol(x), nrows=nrow(x), projs=projs, xmn=xmn, xmx=xmx, ymn=ymn, ymx=ymx)
...
setMethod('layer', signature(x='character'), 
...
setMethod('layer', signature(x='Raster'), 
	function(x, filename="", values=NULL) {
		r <- layer(xmn=xmin(x), xmx=xmax(x), ymn=ymin(x), ymx=ymax(x), nrows=nrow(x), ncols=ncol(x), projs=projection(x))
		filename(r) <- filename
		if (!is.null(values)) {
...
setMethod('layer', signature(x='RasterStack'), 
	function(x, layer=0){
		newindex = -1
		if (nlayers(x) > 0) {
			if (!is.numeric(layer)) {
...
setMethod('layer', signature(x='RasterBrick'), 
	function(x, layer=0){
		newindex = -1
		if (nlayers(x) > 0) {
			if (!is.numeric(layer)) {
		...
		if (layer > 0) {
			...
			if (dataSource(x) == 'disk') {
				if (dindex != layer) { warning(paste("layer was changed to", dindex))}
				r <- layer(filename(x), band=dindex)
				layerNames(r) <- layerNames(x)[dindex]
			} else {
				r <- layer(extent(x), nrows=nrow(x), ncols=ncol(x), projs=projection(x))	
...
setMethod('layer', signature(x='Extent'), 
...
setMethod('layer', signature(x='Spatial'), 
	function(x){
		layer(extent(x))
	}
)
...
setMethod('layer', signature(x='SpatialGrid'), 
	function(x, layer=1){
		r <- layer(extent(x))
...
setMethod('layer', signature(x='SpatialPixels'), 
	function(x, layer=1){
		r <- layer()
		extent(r) <- extent(x)
		projection(r) <- x@proj4string
