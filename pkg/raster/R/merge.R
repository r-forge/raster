...
function(x, y, ..., tolerance=0.05, filename="", format, datatype, overwrite, progress, ext=NULL) { 
	x <- c(x, y, list(...))
	...
	merge(x, tolerance=tolerance, filename=filename, format=format, datatype=datatype, overwrite=overwrite, progress=progress, ext=ext, check=FALSE)
} )



setMethod('merge', signature(x='list', y='missing'), 
function(x, y,..., tolerance=0.05, filename="", format, datatype, overwrite, progress, ext=NULL, check=TRUE){ 
			...
			if (nl > 1) {
				...
				for (i in 1:length(x)) {
					...
					if (xy1[2] > ymin(out) & xy2[2] < ymax(out) & xy1[1] < xmax(out) & xy2[1] > xmin(out)) {		
						cells <- cellsFromExtent( out, extent(x[[i]]) )
						vv <- v[cells]
						vv[is.na(vv)] <- extract(x[[i]], ext)[is.na(vv)]
						v[cells] <- vv
					}
				}
			}
