...
function(x, y, ..., tolerance=0.05, filename="", format, overwrite, progress) { 
	...
	x <- c(x, y, list(...))
	if (missing(format)) { format <- raster:::.filetype(format=format, filename=filename) } 
	if (missing(overwrite)) { overwrite <- raster:::.overwrite()	}
	if (missing(progress)) { progress <- raster:::.progress() }
		...
		for (i in 1:tr$n) {
			...
			for (j in length(x):1) {  #reverse order so that the first raster covers the second etc.
				if (tr$row[i] >= rowcol[j,1] &  tr$row[i] <= rowcol[j,2]) {
					...
					row2 <- row1 + tr$nrow[i] - 1
					nrows <- row2 - row1 + 1
					vv[] <- NA
					vv[, rowcol[j,3]:rowcol[j,4]] <- matrix(getValues(x[[j]], row1, nrows), nrow=nrows, byrow=TRUE)	
		...
		for (i in 1:tr$n) {
			...
			for (j in length(x):1) {  #reverse order so that the first raster covers the second etc.
				if (tr$row[i] >= rowcol[j,1] &  tr$row[i] <= rowcol[j,2]) {
					...
					vv[cells, ] <- getValues(x[[j]], row1, nrows)
					v[!is.na(vv)] <- vv[!is.na(vv)]	
				}
			}
