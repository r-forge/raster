...
In the next section, I discuss some general aspects of the design of the package, notably the structure of the main classes, and what they represent. The use of the package is illustrated in subsequent sections. 
...
\verb@RasterStackBrick@  is a class union of the \verb@RasterStack@ and \verb@RasterBrick@  class. This is a also a virtual class. It allows defining methods that apply to both a \verb@RasterStack@ and to a \verb@RasterBrick@. Like with the Raster class its purpose is efficiency in code and documentation. 
...
<<raster-1>>=
library(raster)
# \verb@RasterLayer@ with the default characteristics
r <- raster()
r
...
# the extract function with a vector of cell numbers
extract(r, 1:10)
...
# create a \verb@RasterBrick@  from file
