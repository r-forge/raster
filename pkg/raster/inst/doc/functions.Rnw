...
<<code-9>>=
f8 <- function(x, a, filename='', progress='', ...) {
	out <- raster(x)
	...
	bs <- blockSize(r)
	pb <- pbCreate(bs$n, type=progress)
...
clusfun <- function(x, filename="", ...) {
	...
	for (i in 1:bs$n) {
		...
		if (filename != "") {
			out <- writeValues(out, d$value$value, bs$row[d$value$tag])
		} else {
			cols <- bs$row[d$value$tag]:(bs$row[d$value$tag]+bs$nrows[d$value$tag]-1)	
...
To use snow with raster, you need to set up a cluster first (only once during a session). After that we can run the function:
