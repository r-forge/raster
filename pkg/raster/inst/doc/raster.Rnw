...
The raster package can work with extremely large files because it does not load the contents of these files into memory. When a Raster object is created form a file, the object gathers some basic information, such as the number of rows and columns, and spatial extent. In computations with a Raster object, data will be read and processes in chunks. If no output filename specified, and the output raster is too large to keep in memory, the results are written to a temporary file. 
