...
r <- raster(system.file("pictures/Rlogo.jpg", package="rgdal"), band=2)
@
...
When used with a RasterLayer as first argument, normal summary statistics functions such as min, max and mean return a RasterLayer. To, instead, obtain a summary for all cells of a single RasterLayer you can use cellStats. You can use {\bf freq} to make a frequency table, or to count the number of cells with a specific value. Use {\bf zonal} to summarize a RasterLayer using zones (areas with the same integer number) defined in another RasterLayer.
