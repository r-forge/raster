...
Many generic methods are implemented for RasterLayer objects, including the R Arith, Math, and Summary 'group-generic' functions. Implementing these functions allows for simple and elegant raster algebra. In raster algebra, the result of a computation is always a RasterLayer. This is perhaps obvious when mutliplying two RasterLayer objects, but perhaps this is not obvious when using functions like sum or mean. In such functions you can mix RasterLayer objects with numbers, as long as the first argument is a RasterLayer.
...
\subsection{overlay}
As an alternative to the raster algebra discussed above, the following 'high-level' functions are available to accomplish the same things: overlay, calc, cover and mask. They provide either easy to use short-hand, or more efficient computation (for disk based RasterLayers). The function calc allows you to do a computation for all the cells of a single layer whereas with overlay you can combine multiple layers. mask removed all values from one layer that are NA in another layer. cover combines two layers by taking the values of the first layer except where these are NA.
...
\section{Plotting}
Several generic functions have been implemented for Raster* objects to create maps and other plot types. 
plot, hist, persp, contour, pairs
click, zoom
